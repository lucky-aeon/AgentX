...
/** 实体冲突检测和合并服务 负责处理分页知识图谱提取中的实体冲突问题
 * 
 * @author shilong.zang */
@Service
public class EntityConflictResolutionService {
    ...
    public List<EntityDto> mergeConflictingEntities(List<EntityDto> entities) {
        ...
        // 按实体ID分组
        Map<String, List<EntityDto>> entitiesById = entities.stream().collect(Collectors.groupingBy(EntityDto::getId));
    ...
    private EntityDto mergeEntityInstances(String entityId, List<EntityDto> instances) {
        ...
        // 收集来源页面信息
        List<Integer> sourcePages = instances.stream().map(EntityDto::getPageNumber).filter(Objects::nonNull).distinct()
                .sorted().collect(Collectors.toList());

        // 创建合并后的实体
        EntityDto mergedEntity = new EntityDto(entityId, new ArrayList<>(mergedLabels), mergedProperties,
                baseEntity.getFileId(), null // 合并实体不特定于某一页
        );
     ...
     * @return 更新后的关系列表 */
    public List<RelationshipDto> updateRelationshipsAfterEntityMerge(List<RelationshipDto> relationships,
        ...
        return relationships.stream().map(rel -> updateRelationshipEntityReferences(rel, entityMergeMap))
                .filter(Objects::nonNull).collect(Collectors.toList());
    }

    /** 更新单个关系的实体引用 */
    private RelationshipDto updateRelationshipEntityReferences(RelationshipDto relationship,
            Map<String, String> entityMergeMap) {
        ...
        // 创建更新后的关系
        RelationshipDto updatedRelationship = new RelationshipDto(updatedSourceId, updatedTargetId,
                relationship.getType(), relationship.getProperties(), relationship.getFileId(),
                relationship.getPageNumber());

        return updatedRelationship;
    }

    /** 生成实体合并映射 用于记录哪些实体被合并了
     * 
     * @param originalEntities 原始实体列表
     * @param mergedEntities 合并后实体列表
     * @return 合并映射 */
    public Map<String, String> generateEntityMergeMap(List<EntityDto> originalEntities,
        ...
        for (EntityDto mergedEntity : mergedEntities) {
            ...
            // 查找所有具有相同ID的原始实体
            originalEntities.stream().filter(original -> original.getId().equals(mergedId))
