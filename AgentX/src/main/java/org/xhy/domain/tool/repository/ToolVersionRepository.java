...
public interface ToolVersionRepository extends MyBatisPlusExtRepository<ToolVersionEntity> {
    ...
    @Select("SELECT * FROM tool_versions t1 " + "WHERE t1.public_status = true "
            + "AND t1.created_at = (SELECT MAX(t2.created_at) FROM tool_versions t2 WHERE t2.tool_id = t1.tool_id AND t2.public_status = true)")
