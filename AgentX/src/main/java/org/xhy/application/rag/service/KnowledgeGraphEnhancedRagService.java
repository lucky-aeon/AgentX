...
/** 知识图谱增强RAG检索应用服务 结合向量搜索和知识图谱查询，提供更准确的检索结果
 * 
 * @author AgentX */
@Service
public class KnowledgeGraphEnhancedRagService {
    ...
    public KgEnhancedRagResponse enhancedRagSearch(KgEnhancedRagRequest request, String userId) {
        ...
        try {
            ...
            log.debug("向量搜索返回 {} 个结果，耗时: {}ms", vectorSearchResult.getResults().size(),
                    vectorSearchResult.getSearchTime());

            // 3. 执行图谱查询（如果启用）
            GraphQueryResult graphQueryResult = executeGraphQuery(request);
            if (graphQueryResult != null) {
                log.debug("图谱查询返回 {} 个实体, {} 个关系，耗时: {}ms", graphQueryResult.getNodes().size(),
                        graphQueryResult.getRelationships().size(), graphQueryResult.getQueryTime());
            }

            // 4. 融合搜索结果
            FusionResult fusionResult = fuseSearchResults(vectorSearchResult, graphQueryResult, request);
            log.debug("结果融合完成，生成 {} 个增强结果，耗时: {}ms", fusionResult.getEnhancedResults().size(),
                    fusionResult.getFusionTime());

            // 5. 重排序和过滤
            List<KgEnhancedRagResponse.EnhancedResult> finalResults = rerankAndFilter(fusionResult.getEnhancedResults(),
                    request);

            // 6. 构建响应
            KgEnhancedRagResponse response = buildResponse(finalResults, vectorSearchResult, graphQueryResult,
                    fusionResult, startTime);

            log.info("知识图谱增强RAG检索完成，最终返回 {} 个结果，处理时间: {}ms", finalResults.size(), response.getProcessingTimeMs());
    ...
    private GraphQueryResult executeGraphQuery(KgEnhancedRagRequest request) {
        ...
        try {
            GraphEntityExtractorService.EntityExtractionResult extractionResult = entityExtractorService
                    .extractEntitiesAndQuery(request.getQuestion(), request.getEntityExtractionStrategy(),
                            request.getMaxGraphDepth(), request.getMaxRelationsPerEntity());

            long queryTime = System.currentTimeMillis() - startTime;

            // 将ExtractedEntity转换为String列表
            List<String> extractedEntityTexts = extractionResult.getExtractedEntities().stream()
                    .map(GraphEntityExtractorService.ExtractedEntity::getText).collect(Collectors.toList());

            return new GraphQueryResult(extractionResult.getGraphNodes(), extractionResult.getGraphRelationships(),
                    extractedEntityTexts, extractionResult.getQueryCount(), queryTime);
        } catch (Exception e) {
    ...
    /** 融合搜索结果 */
    private FusionResult fuseSearchResults(VectorSearchResult vectorResult, GraphQueryResult graphResult,
            KgEnhancedRagRequest request) {
        long startTime = System.currentTimeMillis();

        try {
            List<KgEnhancedRagResponse.EnhancedResult> enhancedResults = hybridSearchStrategy.fuseResults(
                    vectorResult.getResults(), graphResult != null ? graphResult.getNodes() : new ArrayList<>(),
                    graphResult != null ? graphResult.getRelationships() : new ArrayList<>(), request);
    ...
    private KgEnhancedRagResponse buildResponse(List<KgEnhancedRagResponse.EnhancedResult> finalResults,
            VectorSearchResult vectorResult, GraphQueryResult graphResult, FusionResult fusionResult, long startTime) {
    ...
    private List<KgEnhancedRagResponse.EnhancedResult> rerankAndFilter(
            List<KgEnhancedRagResponse.EnhancedResult> results, KgEnhancedRagRequest request) {
        ...
        try {
            ...
            List<KgEnhancedRagResponse.EnhancedResult> filteredResults = results.stream().filter(result -> {
                // 对于图谱增强的结果，使用更宽松的阈值
                ...
                log.debug(
                        "评估结果：sourceType={}, relevanceScore={}, effectiveThreshold={}, hasGraphEnhancement={}, isGraphOnlyResult={}, 通过={}",
                        result.getSourceType(), result.getRelevanceScore(), effectiveThreshold, hasGraphEnhancement,
                        isGraphOnlyResult, passThreshold);

                if (!passThreshold) {
                    log.debug("结果被过滤：相关性分数 {} 低于阈值 {} (图谱增强: {}, 纯图谱: {})", result.getRelevanceScore(),
                            effectiveThreshold, hasGraphEnhancement, isGraphOnlyResult);
                }
            ...
            if (results.isEmpty() && !originalResults.isEmpty()) {
                ...
                // 应用宽松过滤
                results = originalResults.stream().filter(result -> result.getRelevanceScore() >= relaxedThreshold)
                        .collect(Collectors.toList());
    ...
    private String generateContentSignature(String content) {
        ...
        String suffix = content.length() > 50 ? content.substring(Math.max(0, content.length() - 50)) : "";
    ...
    private static class GraphQueryResult {
        ...
        public GraphQueryResult(List<GraphQueryResponse.NodeResult> nodes,
                List<GraphQueryResponse.RelationshipResult> relationships, List<String> extractedEntities,
                int queryCount, long queryTime) {
