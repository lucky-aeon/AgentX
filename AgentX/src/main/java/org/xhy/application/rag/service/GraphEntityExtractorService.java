...
/** 图谱实体提取服务 从查询文本中提取实体并在知识图谱中查找相关信息
 * 
 * @author AgentX */
@Service
public class GraphEntityExtractorService {
    ...
    static {
        // 初始化停用词
        STOP_WORDS.addAll(Arrays.asList("的", "是", "在", "有", "和", "与", "或", "但", "然而", "因为", "所以", "如何", "什么", "哪里",
                "为什么", "怎么", "能够", "可以", "应该", "需要", "关于", "对于", "通过", "根据", "按照", "依据", "基于", "由于", "为了", "这个", "那个",
                "这些", "那些", "它们", "我们", "你们", "他们", "她们"));

        // 初始化技术关键词
        TECHNICAL_KEYWORDS.addAll(Arrays.asList("算法", "模型", "系统", "平台", "架构", "框架", "技术", "方法", "策略", "服务", "接口", "API",
                "数据库", "缓存", "队列", "消息", "事件", "流程", "配置", "部署", "监控", "日志", "异常", "错误", "性能", "优化", "扩展"));

        // 初始化实体识别模式
        ENTITY_PATTERNS.put("PERSON", Arrays.asList(Pattern.compile("[\\u4e00-\\u9fa5]{2,4}(?:先生|女士|老师|教授|博士|工程师)?"),
                Pattern.compile("\\b[A-Z][a-z]+\\s+[A-Z][a-z]+\\b")));

        ENTITY_PATTERNS.put("ORGANIZATION", Arrays.asList(Pattern.compile("[\\u4e00-\\u9fa5]+(?:公司|集团|企业|机构|组织|部门|团队)"),
                Pattern.compile("\\b[A-Z][a-zA-Z]*\\s*(?:Inc|Corp|Ltd|Company|Organization)\\b")));

        ENTITY_PATTERNS.put("TECHNOLOGY",
                Arrays.asList(Pattern.compile("(?:Java|Python|JavaScript|React|Spring|MySQL|Redis|Docker|Kubernetes)"),
                        Pattern.compile("[A-Z][a-z]*(?:Service|Controller|Repository|Manager|Handler|Processor)")));

        ENTITY_PATTERNS.put("CONCEPT",
                Arrays.asList(Pattern.compile("[\\u4e00-\\u9fa5]{2,8}(?:系统|平台|架构|框架|模式|策略|方案)")));
    }
    ...
    public EntityExtractionResult extractEntitiesAndQuery(String question,
            KgEnhancedRagRequest.EntityExtractionStrategy strategy, int maxDepth, int maxRelationsPerEntity) {
        ...
        try {
            ...
            log.debug("实体提取完成，找到 {} 个图谱节点, {} 个关系，执行了 {} 次查询", graphNodes.size(), graphRelationships.size(),
                    queryCount);
    ...
    /** 基于LLM的实体提取 使用大语言模型从文本中智能提取实体信息 支持多种实体类型识别和置信度评估 */
    private Set<ExtractedEntity> extractEntitiesByLLM(String text) {
        try {
            ...
            ProviderConfig entityExtractionProviderConfig = new ProviderConfig(
                    "sk-b9aL4HqXa1OHY6TyctHBVnjq9IQndYd5snq4WdX9sQ4DUFma", "https://new.281182.xyz/v1",
                    "gemini-2.5-pro", ProviderProtocol.OPENAI);

            ChatModel entityExtractionModel = LLMProviderService.getStrand(ProviderProtocol.OPENAI,
                    entityExtractionProviderConfig);
    ...
    private Set<ExtractedEntity> parseEntityExtractionResponse(String responseText) {
        ...
        try {
            ...
            // 2. 解析JSON
            EntityExtractionResponse response = Neo4jCompatibleJsonParser.parseObject(cleanedJson,
                    EntityExtractionResponse.class);
            ...
            if (response != null && response.getEntities() != null) {
                ...
                for (EntityExtractionResponse.EntityItem item : response.getEntities()) {
                    if (item.getText() != null && !item.getText().trim().isEmpty()) {
                        ExtractedEntity entity = new ExtractedEntity(item.getText().trim(),
                                item.getType() != null ? item.getType() : "UNKNOWN", 0, 0);
    ...
    /** 从AI返回的文本中提取JSON内容 处理可能包含Markdown代码块或其他格式的文本 */
    ...
    /** 在知识图谱中查询实体 */
    private void queryEntityInGraph(ExtractedEntity entity, List<GraphQueryResponse.NodeResult> graphNodes,
            List<GraphQueryResponse.RelationshipResult> graphRelationships, int maxDepth, int maxRelationsPerEntity) {
        try {
            // 1. 精确匹配查询
            GraphQueryResponse exactMatch = graphQueryAppService.findNodesByProperty("GenericNode", "name",
                    entity.getText(), 5);
            ...
            if (!fuzzyMatches.isEmpty()) {
                ...
                for (GraphQueryResponse.NodeResult node : fuzzyMatches) {
                    GraphQueryResponse relationResults = graphQueryAppService.findNodeRelationships(node.getId(), null,
                            "BOTH", maxRelationsPerEntity);
    ...
    /** 处理图谱查询结果 */
    private void processGraphQueryResult(GraphQueryResponse queryResult, List<GraphQueryResponse.NodeResult> graphNodes,
            List<GraphQueryResponse.RelationshipResult> graphRelationships, int maxRelationsPerEntity) {
    ...
    public static class ExtractedEntity {
        ...
        public String toString() {
            return "ExtractedEntity{" + "text='" + text + '\'' + ", type='" + type + '\'' + ", confidence=" + confidence
                    + '}';
        }
    }
