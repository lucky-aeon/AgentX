...
/** 文档结构化服务 负责在所有页面处理完成后，创建文档级的主节点和层次结构
 * 
 * @author AgentX */
@Service
public class DocumentStructuringService {
    ...
    public GraphIngestionResponse createDocumentStructure(String documentId, List<EntityDto> allEntities,
            List<RelationshipDto> allRelationships, Integer totalPages) {

        logger.info("开始为文档 {} 创建结构化知识图谱，实体数: {}, 关系数: {}, 页数: {}", documentId, allEntities.size(),
                allRelationships.size(), totalPages);
        ...
        try {
            ...
            // 2. 使用AI生成文档结构
            DocumentStructureResult structureResult = generateDocumentStructureWithAI(documentId, allEntities,
                    allRelationships, analysis, totalPages);

            // 3. 创建最终的图谱请求
            GraphIngestionRequest finalRequest = buildFinalGraphRequest(documentId, structureResult, allEntities,
                    allRelationships);

            // 4. 保存到Neo4j
            GraphIngestionResponse response = graphIngestionAppService.ingestGraphData(finalRequest);

            logger.info("文档 {} 结构化完成，新增主节点: {}, 主题节点: {}, 层次关系: {}", documentId, 1, structureResult.topicNodes.size(),
                    structureResult.hierarchicalRelationships.size());
    ...
    /** 分析文档结构 */
    private DocumentStructureAnalysis analyzeDocumentStructure(List<EntityDto> entities,
            List<RelationshipDto> relationships, Integer totalPages) {

        DocumentStructureAnalysis analysis = new DocumentStructureAnalysis();

        // 分析实体重要性分布
        Map<String, Long> entityTypeDistribution = entities.stream().flatMap(entity -> entity.getLabels().stream())
                .collect(Collectors.groupingBy(label -> label, Collectors.counting()));

        // 分析页面分布
        Map<Integer, Long> pageDistribution = entities.stream().filter(entity -> entity.getPageNumber() != null)
                .collect(Collectors.groupingBy(EntityDto::getPageNumber, Collectors.counting()));

        // 识别高频实体（可能的主题）
        Map<String, Long> entityFrequency = entities.stream()
                .collect(Collectors.groupingBy(entity -> entity.getProperties().get("name") != null
                        ? entity.getProperties().get("name").toString()
                        : entity.getId(), Collectors.counting()));
        ...
        logger.debug("文档结构分析完成，实体类型: {}, 高频实体: {}", entityTypeDistribution.size(), entityFrequency.size());

        return analysis;
    }

    /** 使用AI生成文档结构 */
    private DocumentStructureResult generateDocumentStructureWithAI(String documentId, List<EntityDto> entities,
            List<RelationshipDto> relationships, DocumentStructureAnalysis analysis, Integer totalPages) {
    ...
    private String createEntitySummary(List<EntityDto> entities, DocumentStructureAnalysis analysis) {
        ...
        // 高重要性实体
        List<EntityDto> importantEntities = entities.stream().filter(entity -> {
        ...
        for (EntityDto entity : importantEntities) {
            summary.append("- ").append(entity.getId()).append(" (").append(String.join(",", entity.getLabels()))
                    .append(")").append(": ").append(entity.getProperties().get("name")).append("\n");
        }
    ...
    private String createRelationshipSummary(List<RelationshipDto> relationships, DocumentStructureAnalysis analysis) {
        ...
        analysis.relationshipTypeDistribution.entrySet().stream()
                .sorted(Map.Entry.<String, Long>comparingByValue().reversed()).limit(5).forEach(entry -> summary
    ...
    private DocumentStructureResult parseStructuringResponse(String aiResponse) {
        try {
            ...
            if (responseMap.containsKey("topicNodes")) {
                ...
                result.topicNodes = topicMaps.stream().map(this::parseEntityFromMap).collect(Collectors.toList());
            }

            // 解析层次关系
            if (responseMap.containsKey("hierarchicalRelationships")) {
                List<Map<String, Object>> relationshipMaps = (List<Map<String, Object>>) responseMap
                        .get("hierarchicalRelationships");
                result.hierarchicalRelationships = relationshipMaps.stream().map(this::parseRelationshipFromMap)
    ...
    /** 创建默认文档结构（AI失败时的备选方案） */
    private DocumentStructureResult createDefaultDocumentStructure(String documentId, List<EntityDto> entities,
        ...
        // 创建基本的层次关系
        result.hierarchicalRelationships = createDefaultHierarchicalRelationships(documentNode, result.topicNodes,
                entities);
    ...
    private List<EntityDto> createDefaultTopicNodes(String documentId, DocumentStructureAnalysis analysis) {
        ...
        analysis.entityTypeDistribution.entrySet().stream()
                .sorted(Map.Entry.<String, Long>comparingByValue().reversed()).limit(3) // 最多3个主题
                .forEach(entry -> {
    ...
    /** 创建默认层次关系 */
    private List<RelationshipDto> createDefaultHierarchicalRelationships(EntityDto documentNode,
            List<EntityDto> topicNodes, List<EntityDto> entities) {
        ...
        for (EntityDto topicNode : topicNodes) {
            ...
            entities.stream().filter(entity -> entity.getLabels().contains(topicType)).limit(10) // 每个主题最多连接10个实体
                    .forEach(entity -> {
    ...
    /** 构建最终的图谱请求 */
    private GraphIngestionRequest buildFinalGraphRequest(String documentId, DocumentStructureResult structureResult,
