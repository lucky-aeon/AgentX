...
/** 文档级图谱合并服务 负责协调整个文档的分页图谱提取和合并过程
 * 
 * @author shilong.zang */
@Service
public class DocumentGraphMergeService {
    ...
    public DocumentGraphMergeService(EntityConflictResolutionService conflictResolutionService,
            GraphIngestionAppService graphIngestionAppService, DocumentStructuringService documentStructuringService) {
    ...
    public GraphIngestionResponse processPagedGraph(GraphIngestionRequest request, Integer pageNumber,
            Integer totalPages) {
        ...
        if (buffer.isAllPagesReceived()) {
            ...
            return GraphIngestionResponse.partial(documentId,
                    ...
                    request.getRelationships() != null ? request.getRelationships().size() : 0, "页面数据已缓存，等待其他页面完成");
        }
    }
    ...
    private GraphIngestionResponse mergeAndIngestDocumentGraph(String documentId, DocumentGraphBuffer buffer) {
        try {
            ...
            // 生成实体合并映射
            Map<String, String> entityMergeMap = conflictResolutionService.generateEntityMergeMap(allEntities,
                    mergedEntities);
            ...
            // 使用文档结构化服务创建主节点和层次结构
            GraphIngestionResponse response = documentStructuringService.createDocumentStructure(documentId,
                    mergedEntities, updatedRelationships, buffer.getTotalPages());

            logger.info("文档 {} 图谱合并完成，原始实体数: {}, 合并后实体数: {}, 关系数: {}", documentId, allEntities.size(),
                    mergedEntities.size(), updatedRelationships.size());
    ...
    private DocumentGraphBuffer getOrCreateDocumentBuffer(String documentId, Integer totalPages) {
        return documentBuffers.computeIfAbsent(documentId, id -> new DocumentGraphBuffer(id, totalPages));
    }
    ...
    /** 强制完成文档处理（即使有页面缺失） 用于处理部分页面失败的情况 */
    ...
    private static class DocumentGraphBuffer {
        ...
        public List<EntityDto> getAllEntities() {
            return pageEntities.values().stream().flatMap(List::stream).collect(Collectors.toList());
        }

        public List<RelationshipDto> getAllRelationships() {
            return pageRelationships.values().stream().flatMap(List::stream).collect(Collectors.toList());
        }
