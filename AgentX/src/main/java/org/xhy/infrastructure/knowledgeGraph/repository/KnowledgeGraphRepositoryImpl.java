...
/** 知识图谱仓储实现类 基于Neo4j实现知识图谱数据的持久化操作
 * 
 * @author zang
 * @since 1.0.0 */
@Repository
public class KnowledgeGraphRepositoryImpl implements KnowledgeGraphRepository {
    ...
    /** 摄取知识图谱数据 批量存储实体和关系数据到Neo4j图数据库
     * 
    ...
    public GraphIngestionResponse ingestGraphData(GraphIngestionRequest request) {
        ...
        try {
            ...
            // 创建响应
            GraphIngestionResponse response = GraphIngestionResponse.success(request.getDocumentId(), entitiesProcessed,
    ...
    public GraphQueryResponse executeQuery(GraphQueryRequest request) {
        ...
        try {
            ...
            List<Map<String, Object>> rawResults = new ArrayList<>(
                    neo4jClient.query(queryResult.getCypher()).bindAll(queryResult.getParameters()).fetch().all());
    ...
    /** 查找两个节点之间的路径 使用最短路径算法查找节点间的连接路径
     * 
    ...
    public GraphQueryResponse findPathBetweenNodes(String sourceNodeId, String targetNodeId, Integer maxDepth) {
        ...
        try {
            ...
            Map<String, Object> params = Map.of("sourceId", sourceNodeId, "targetId", targetNodeId);

            List<Map<String, Object>> rawResults = new ArrayList<>(
                    neo4jClient.query(pathQuery).bindAll(params).fetch().all());
    ...
    /** 获取知识图谱统计信息 统计图中的节点数量、关系数量、标签类型等信息
     * 
    ...
    public Map<String, Object> getGraphStatistics() {
        try {
            ...
            Map<String, Object> result = neo4jClient.query(statsQuery).fetch().one().orElse(new HashMap<>());
    ...
    public boolean existsNode(String nodeId) {
        try {
            ...
            Map<String, Object> result = neo4jClient.query(existsQuery).bind(nodeId).to("nodeId").fetch().one()
    ...
    /** 删除指定文档的所有图数据 清理与指定文档相关的所有节点和关系
     * 
    ...
    public Map<String, Integer> deleteGraphDataByDocument(String documentId) {
        try {
            ...
            Map<String, Object> countResult = neo4jClient.query(countQuery).bind(documentId).to("documentId").fetch()
                    .one().orElse(Map.of("nodeCount", 0L, "relationshipCount", 0L));
            ...
            neo4jClient.query(deleteQuery).bind(documentId).to("documentId").run();
    ...
    /** 批量摄取实体 使用UNWIND和MERGE实现高效的批量插入/更新
     * 
     * @param entities 实体列表
     * @return 处理的实体数量 */
    private int ingestEntities(List<GraphIngestionRequest.EntityDto> entities) {
        ...
        try {
            ...
            neo4jClient.query(entityQuery).bindAll(Map.of("entities", entityParams)).run();
    ...
    /** 批量摄取关系 使用UNWIND和MERGE实现高效的关系创建
     * 
    ...
    private int ingestRelationships(List<GraphIngestionRequest.RelationshipDto> relationships) {
        ...
        List<Map<String, Object>> relationshipParams = relationships.stream()
                .map(rel -> Map.of("sourceId", rel.getSourceId(), "targetId", rel.getTargetId(), "type", rel.getType(),
                        "properties",
                        Neo4jValueConverter
        ...
        try {
            ...
            neo4jClient.query(relationshipQuery).bindAll(Map.of("relationships", relationshipParams)).run();
    ...
    private void mapQueryResults(List<Map<String, Object>> rawResults, List<GraphQueryResponse.NodeResult> nodes,
            List<GraphQueryResponse.RelationshipResult> relationships, GraphQueryRequest.ReturnDefinition returnDef) {
